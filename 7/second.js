...
const checkOperation = (ingredients, currentResult, expectedResult) => {
    ...
    const concatenationResult = parseInt(currentResult.toString() + ingredients[0].toString())
    const wasConcatenationCorrect = checkOperation(ingredients.slice(1), concatenationResult, expectedResult);

    if (wasConcatenationCorrect) {
        return true;
    }
